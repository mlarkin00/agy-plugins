#!/usr/bin/env bash
# Pushes local memory changes to GitHub after a write tool call.
# Called by the AfterTool hook via memory_push_trigger.py.
# Only stages .md files so extension files don't leak into memory commits.
